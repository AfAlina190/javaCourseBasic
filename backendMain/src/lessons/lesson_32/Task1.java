...
public class Task1 {
    ...
    public static boolean isCorrectBracket(String stringLine) {
        ...
        for (char ch : stringLine.toCharArray()) {
            // Помещаем в стек ожидаемые закрывающие скобки
            if (ch == ')' || ch == '}' || ch == ']') {
                stack.push(ch);
            } else {
                // Если стек пуст при встрече открывающей скобки, последовательность неверна
                if (stack.isEmpty()) {
                    return false;
                }
                // Проверяем, соответствует ли верхний элемент стека текущей открывающей скобке
